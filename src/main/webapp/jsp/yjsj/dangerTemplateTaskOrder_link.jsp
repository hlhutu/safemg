	...
	$(window).ready(function() {
		...
		ajax(obj, function(result) { // 查询业务数据
			if ("error" == result.resCode) {
				...
				//表格填充
				var contentList = result.datas[0].contentList;
				$.each(contentList,function(index,row){
					row.theme = row.parentContent[row.parentContent.length-1]
				});
				$("#data_table").bootstrapTable("load",contentList);
			}
		});
		//初始化表格框
		var params = {
			columns: [
				{
					...
					formatter: function(value, row, index){
						if(value == 0){ 
							return '<span style="color:#F24141">异常</span>';
						}
						if(value == 1){ 
		...
		initBootStrapTable($("#data_table"),params);
	});
				...
				<div class="ibox float-e-margins">
					<form id="thisform" class="form-horizontal" role="form" onsubmit="return false;">
						<div class="ibox-title">
							<h5>
								<class="fa fa-file-o a-clear"></i> 执法项执行列表
							</h5>
						...
						<div class="ibox-content"> 
							  <div class="form-group" id="list_file_div">
							    <div class="col-sm-12">
							      <table id="data_table" class="table" style="width: 100%"></table>
							    </div>
							  </div>
						</div>
					</form>
