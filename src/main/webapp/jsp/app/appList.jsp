...
<script>
	var selectUrl = "${ctx}/biz/app/select.do";
	var insertUrl = "${ctx}/biz/app/insert.do";
	var updateUrl = "${ctx}/biz/app/update.do";
	var deleteUrl = "${ctx}/biz/app/delete.do";

	var qrcodeSelectUrl = "${basePath}/api/biz/app/qrcode.do";
	var qrcodeUrl = "${ctx}/images/barcode.png";
	...
	$(window).ready(function() {
		...
		var table = $("#datas").dataTable({
			...
			},
			{
				...
				"title":"操作",
				"render": function (data, type, row) {
					var str = '<a class="info">详情</a>';
					str += '  <a class="qr_code">二维码</a>';
					return str;
				}
			}]
			,"drawCallback": function(s) {
				...
				$(".qr_code").off("click").on("click", function () {
					var r = table.api().row($(this).parents('tr')).data();
					window.open(qrcodeUrl+"?text="+qrcodeSelectUrl+"?version="+r.version);
				});
			}
		});
		...
		$("#qr-code").on("click", function() {
			window.open(qrcodeUrl+"?text="+qrcodeSelectUrl);
		});
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-title">
						<h5>
							<i class="fa fa-file-o a-clear"></i> 版本更新
						</h5>
					...
					<div class="ibox-content">
						...
						<div class="form-horizontal clearfix">
							<div class="col-sm-6 pl0">
								<s:access url="${basePath}/biz/app/insert.do">
									<button class="btn btn-sm btn-success" id="btn-insert">
										<i class="fa fa-plus"></i> 新增
									</button>
								</s:access>
								<s:access url="${basePath}/biz/app/update.do">
									<button class="btn btn-sm btn-success" id="btn-update">
										<i class="fa fa-edit"></i> 修改
									</button>
								</s:access>
								<s:access url="${basePath}/biz/app/delete.do">
									<button class="btn btn-sm btn-success" id="btn-delete">
										<i class="fa fa-trash-o"></i> 删除
									</button>
								</s:access>
								<button class="btn btn-sm btn-success" id="qr-code">
				...
				<form id="editForm" class="form-horizontal key-13" role="form" onsubmit="return false;">
					...
					<div class="ibox-content">
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<input type="text" class="input-sm form-control validate[required]" name="version" id="version">
							</div>
							<label class="col-sm-2 control-label">apk</label>
							<div class="col-sm-4">
								<input type="file" class="input-sm form-control validate[required]" name="file" id="file">
							</div>
						</div>
						<div class="form-group">
